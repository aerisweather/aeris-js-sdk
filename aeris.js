...
* @version $Id: aeris.js 1094 2012-10-04 01:00:01Z nshipes $
...
(function(Aeris){
	...
	var root = this;
	// function to check if the Aeris property is already set from user initialization
	var hasProp = function(prop) {
		return (root && root.Aeris && root.Aeris[prop]);
	};
	var getProp = function(prop) {
		if (!hasProp(prop)) return;
		return root.Aeris[prop];
	}
	...
	var Aeris = root.Aeris = {
		...
		init: function(options) {
			...
			this.config = {
				...
				client_secret: null,
				paths: {
					wxicons: ((Aeris.secure) ? 'https://js-secure' : 'http://js') + '.aerisapi.com/img/'
				},
				links: {
					wxlocal: 'http://wx.hamweather.com/'
				},
				widgets: false,
				dependencies: null,
				debug: false
			...
			// add versioning to wxicon path if needed
			if (this.loadVersion !== undefined)
                this.config.paths.wxicons = ((this.secure) ? 'https://js-secure' : 'http://js') + '.aerisapi.com/' + this.loadVersion + '/img/';
		...
		onLoad: (hasProp('onLoad')) ? getProp('onLoad') : function() {
			//console.log('No implementation of Aeris.onLoad found!');
		},

		onReady: (hasProp('onReady')) ? getProp('onReady') : function() {
			//console.log('No implementation of Aeris.onReady found!');
		},

		loadVersion: (hasProp('version')) ? getProp('version') : undefined,
		secure: (hasProp('secure')) ? getProp('secure') : false,
		base: (hasProp('base')) ? getProp('base') : undefined
	};

	// current version of the library
	Aeris.VERSION = '1.2.2';
	// set base path for Aeris.require to use
	Aeris.jsBase = (Aeris.base !== undefined) ? Aeris.base : ((Aeris.secure) ? 'https://js-secure' : 'http://js') + '.aerisapi.com';




	//-------------------------------
	// Core Require JS source
	//-------------------------------
	...
	//-------------------------------
	// END Require JS source
	//-------------------------------


	// push require JS into Aeris namespace
	Aeris.require = Aeris.requirejs = req;
	Aeris.define = define;
	...
	var loadDependencies = function() {
		...
		var min = (Aeris.config.debug == true || Aeris.userConfig.debug == true) ? '' : '.min';
		...
		if (typeof jQuery === 'undefined') {
			libsToLoad.push(order + 'jquery');
		}
		// always load jQuery easing extension
		libsToLoad.push(order + 'jquery.easing');

		if (typeof _ === 'undefined') {
			libsToLoad.push(order + 'underscore');
		}

		// add our own dependencies
		libsToLoad.push(order + 'handlebars');
		libsToLoad.push(order + 'aeris.core');

		//console.log('loading js dependencies: '+libsToLoad.join(', '));
	...
	window.setTimeout(function(){
		if (window.Aeris.onLoad) window.Aeris.onLoad();
	}, 0);
